...
async def lifespan(app: FastAPI):
    ...
    skills_dir = os.getenv("EMOASS_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills"))
    _skill_manager = SkillManager(
        root_dir=skills_dir,
        max_prompt_chars=int(os.getenv("EMOASS_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    _skill_manager.load()
    ...
    logger.info("EmoAss 已就绪")
    yield

    await _monitor.stop()
    logger.info("EmoAss 已关闭")
...
async def _cli():
    ...
    skill_manager = SkillManager(
        root_dir=os.getenv("EMOASS_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills")),
        max_prompt_chars=int(os.getenv("EMOASS_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    skill_manager.load()
    ...
    while True:
        ...
        print(f"\nEmoAss [{result.agent_type.value}]: {result.response}\n")
